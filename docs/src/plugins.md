...
Pass `plugins=(AdaptiveMutationWeightsPlugin(adaptation_strength=0),)` to
disable only mutation-weight adaptation, or `default_plugins=()` to disable
all automatic plugins.
